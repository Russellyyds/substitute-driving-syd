...
//        第一种方式根据订单id获取代驾订单位置信息，根据创建时间升序排序
//        OrderServiceLocation orderServiceLocation = new OrderServiceLocation();
...
//        List<OrderServiceLocation> all = orderServiceLocationRepository.findAll(example, createTime);
        double realDistance = 0;
        //第二种方式
        List<OrderServiceLocation> list = orderServiceLocationRepository.findByOrderIdOrderByCreateTimeAsc(orderId);
        if (!CollectionUtils.isEmpty(list)) {
            for (int i = 0; i < list.size() - 1; i++) {
                OrderServiceLocation orderServiceLocation = list.get(i);
                OrderServiceLocation orderServiceLocation1 = list.get(i + 1);
                double distance = LocationUtil.getDistance(
                        orderServiceLocation.getLatitude().doubleValue(),
                        orderServiceLocation.getLongitude().doubleValue(),
                        orderServiceLocation1.getLatitude().doubleValue(),
